...
function convert() {
    ...
    switch (conversionType) {
        ...
        case 'clearhtml':
            ...
            break;
            case 'transgerman':
            document.getElementById('outputText').value = transliterateGermanToUkrainian(inputText);
            break;
            case 'transpolish':
...
function ToUpperCase() {
    const inputText = document.getElementById('inputText').value.toUpperCase();
    const uppercase = convertUpperCase(inputText);
    document.getElementById('outputText').value = uppercase;
}
function ToLowerCase() {
    const inputText = document.getElementById('inputText').value.toUpperCase();
    const lowerCase = convertLowerCase(inputText);
    document.getElementById('outputText').value = lowerCase;
}

function ToUpperLowerCase() {
    const inputText = document.getElementById('inputText').value.toUpperCase();
    const upperlowerCase = onvertUpperLowerCase(inputText);
    document.getElementById('outputText').value = upperlowerCase;
}
...
function transliterateGermanToUkrainian(text) {
    const transliterationMap = {
'a': 'а', 'aa': 'а', 'ah': 'а', 'аі': 'ай', 'au': 'ау', 'ä': 'е', 'e': 'е', 'ee': 'е', 'eh': 'е', 'ei': 'ай',
'eu': 'ой', 'f': 'ф', 'g': 'г/ґ', 'h': 'г', 'i': 'й', 'і': 'й', 'ie': 'і/и', 'ih': 'і/и', 'j': 'й', 'k': 'к',
'l': 'л', 'ль': 'ль', 'm': 'м', 'n': 'н', 'ng': 'нг', 'nk': 'нк', 'o': 'о', 'oh': 'о', 'oo': 'о', 'ö': 'е',
'p': 'п', 'ph': 'ф', 'qu': 'кв', 'r': 'р', 's': 'ш', 'з': 'ш', 'с': 'с', 'sch': 'ш', 'ss': 'с', 'ß': 'с',
't': 'т', 'th': 'т', 'tsch': 'ч', 'tz': 'ц', 'u': 'у', 'uh': 'у', 'ü': 'ю', 'üh': 'ю, і', 'v': 'в', 'ф': 'ф',
'w': 'в', 'x': 'кс', 'y': 'і/и', 'й': 'й', 'z': 'ц'
};
...
const currentChunkPlusNextChar = currentChunk + nextChar;

if (transliterationMap[currentChunkPlusNextChar]) {
    currentChunk = currentChunkPlusNextChar;
    i++; // Skip the next character
} else if (transliterationMap[currentChunk]) {
    transliteratedText += transliterationMap[currentChunk];
...
if (transliterationMap[currentChunk]) {
transliteratedText += transliterationMap[currentChunk];
} else {
transliteratedText += currentChunk;
}
